...
Возможно при использовании панели вам и не прийдеться использовать этот режим, но суть его в том, что он просто выведет значение поля, которое было ему присвоено изначально без дополнительных модификаций.
...
Вы используете поле Select c опциями ссылок на изображения и хотите в режиме preview выводить не ссылки, а сразу рендерить изображения, результат будет достигнут за счет метода `changePreview`, ваш код будет выглядеть следующим образом:
...
В процессе мы также столкнулись с методом fill и наполнили поле, но если мы используем его в готовом ModelResource или FormBuilder, то поле будет наполнено за нас и вызванный нами fill будет перезаписан, поэтому в ваших задачах может возникнуть ситуация, когда вам потребуется изменить логику наполнения, интегрироваться в этот процесс, для этого вам помогут методы `changeFill` и `afterFill`.
